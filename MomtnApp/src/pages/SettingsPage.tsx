...
const SettingsPage = () => {
  ...
  useEffect(() => {
    if (user) {
      // Load greeting from user metadata (syncs with mobile)
  ...
  const saveGreeting = async () => {
    ...
    setIsUpdatingName(true); // استخدام نفس مؤشر التحميل
    try {
      const { error } = await supabase.auth.updateUser({
        data: { greeting_message: greetingMessage }
      });
      ...
      toast({
        title: "خطأ",
        description: "حدث خطأ أثناء حفظ رسالة الترحيب",
        variant: "destructive"
      });
    } finally {
